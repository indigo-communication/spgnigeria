...
foreach ($page in $pages) {
    ...
    try {
        ...
        if ($response.StatusCode -eq 200) {
            ...
            Write-Host "✓ $page - $size KB" -ForegroundColor Green
            ...
            Write-Host "⚠ $page - Status: $($response.StatusCode)" -ForegroundColor Yellow
            $results += [PSCustomObject]@{
                Page = $page
...
if ($okCount -eq $totalCount) {
    ...
    Write-Host "`n⚠ SOME PAGES FAILED TO LOAD" -ForegroundColor Yellow
}
